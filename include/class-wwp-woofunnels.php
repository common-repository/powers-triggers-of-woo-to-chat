<?php
class Woowhatspowers_Woofunnels {
	private $acoes = array();
	...
	function wpp_load_acoes(){
		$acoes = get_option( 'wwp_woofunnels_acoes', '' );
		if (!empty($acoes)) {
			$this->acoes = $acoes;
		}
	}
	...
	function wpp_saveAcoes($dados = array()){
		...
		$up = update_option('wwp_woofunnels_acoes', $salvar,FALSE);
		if (!$up) {
			$up = add_option('wwp_woofunnels_acoes', $salvar);
		}
		$this->wpp_load_acoes();
		return $up;
	}
